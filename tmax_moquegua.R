...
for (i in 1:length(unique(final_table$Fecha))){
  ...
  #####Eliminando NAs
  final_table2 <- final_table2[is.na(final_table2$Tmax) ==F,]
  ...
  v_emp_ok = variogram(Tmax ~ 1, sf)
  
  library(automap)
  v_mod_ok = autofitVariogram(Tmax ~ 1, sf)
  ...
  g = gstat(formula = Tmax ~ 1, model = v_mod_ok$var_model, data = sf)
  z = predict(g,sf2)
...
data <- rast("C:/Users/ASUS/Downloads/chirts_tmax (1).nc")
data
data[data <=-99] <- NA
...
g1 <- ggplot(data=shp_caj)+
  ...
  scale_fill_gradientn(colours=(paletteer_c("grDevices::Geyser", 30)),breaks=seq(0.4,13,0.5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  ...
  ggtitle("RMSE Kriging y Spline - Chirts : Temperatura Máxima")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
...
ggsave(g1,filename=paste0("map_RMSE_spline__kriging_tmax.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  geom_sf(fill="transparent",size=1,col="black")+facet_wrap(~Interpolacion*Mes_lab,nrow=4,dir="v")  +
  guides(fill=guide_colorsteps()) +
  scale_fill_gradientn(colours=(paletteer_c("ggthemes::Sunset-Sunrise Diverging", 30)),breaks=seq(0.3,13,0.5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  theme(panel.grid.major = element_line(color="black", linetype="dashed", size=0.25),legend.key.width  = unit(4,"cm"),
        panel.ontop = TRUE,
        panel.background = element_rect(color=NA, fill=NA),  
  ...
  ggtitle("MAE Kriging y Spline - Chirps : Temperatura Máxima")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MAE_spline__kriging_tmax.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  scale_fill_gradientn(colours=rev(
    paletteer_c("grDevices::GnBu", 30)),breaks=seq(0.2,13,0.5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  theme(panel.grid.major = element_line(color="black", linetype="dashed", size=0.25),legend.key.width  = unit(4,"cm"),
        panel.ontop = TRUE,
        panel.background = element_rect(color=NA, fill=NA),  
  ...
  ggtitle("MAD Kriging y Spline - Chirps : Temperatura Máxima")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MAd_spline__kriging_tmax.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  guides(fill=guide_colorsteps()) +
  scale_fill_gradientn(colours=rev(
    paletteer_c("grDevices::Earth", 30)),breaks=seq(0.1,172,10)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-16))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  ...
  ggtitle("MSE Kriging y Spline - Chirts : Temperatura Máxima")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MSE_spline__kriging_tmax.png"), width = 10.5, height =10,dpi=700)
