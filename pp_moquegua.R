...
for (i in 1:length(datos_full)){
  data <- read.table(datos_full[i],na.strings = c(-99.9,-999,-888,888,999,"NA","S/D"))
  ...
  data$Tmed <- (data$Tmax + data$Tmin)/2    ### Temperatura media
  
  data <- data %>% filter(fecha >= as.Date("2003-01-01") & fecha <= as.Date("2012-12-31"))%>%  group_by(Año,Mes) %>% summarise(Pp= sum_na(Pp),Tmed= mean_na(Tmed),Tmax=mean_na(Tmax),Tmin= mean_na(Tmin))
...
for (i in 1:length(unique(final_table$Fecha))){
  ...
  #####Eliminando NAs
  final_table2 <- final_table2[is.na(final_table2$Pp) ==F,]
  
  ######Creando rejilla a interpolar
  y = seq(-17.975,-15.825,0.05)
  x = seq(-71.625,-69.825,0.05)
  length(y)
  length(x)
  ...
  v_emp_ok = variogram(Pp ~ 1, sf)
  
  library(automap)
  v_mod_ok = autofitVariogram(Pp ~ 1, sf)
  ...
  g = gstat(formula = Pp ~ 1, model = v_mod_ok$var_model, data = sf)
  z = predict(g,sf2)
...
month(df2_data$Fecha,label=T,abbr=F)
...
g1 <- ggplot(data=shp_caj)+
  ...
  scale_fill_gradientn(colours=(paletteer_c("grDevices::Geyser", 30)),breaks=seq(0.5,90,5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  ...
  ggtitle("RMSE Kriging y Spline - Chirps : Precipitación")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
...
ggsave(g1,filename=paste0("map_RMSE_spline__kriging_pp.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  geom_sf(fill="transparent",size=1,col="black")+facet_wrap(~Interpolacion*Mes_lab,nrow=4,dir="v")  +
  guides(fill=guide_colorsteps()) +
  scale_fill_gradientn(colours=(paletteer_c("ggthemes::Sunset-Sunrise Diverging", 30)),breaks=seq(0.2,90,5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  theme(panel.grid.major = element_line(color="black", linetype="dashed", size=0.25),legend.key.width  = unit(4,"cm"),
        panel.ontop = TRUE,
        panel.background = element_rect(color=NA, fill=NA),  
  ...
  ggtitle("MAE Kriging y Spline - Chirps : Precipitación")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MAE_spline__kriging_pp.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  scale_fill_gradientn(colours=rev(
    paletteer_c("grDevices::GnBu", 30)),breaks=seq(0.2,94,5)) +theme_bw()+coord_sf(xlim=c(-72,-69.5), ylim=c(-18,-15.5))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  theme(panel.grid.major = element_line(color="black", linetype="dashed", size=0.25),legend.key.width  = unit(4,"cm"),
        panel.ontop = TRUE,
        panel.background = element_rect(color=NA, fill=NA),  
  ...
  ggtitle("MAD Kriging y Spline - Chirps : Precipitación")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-72, -69.5, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MAd_spline__kriging_pp.png"), width = 11, height =10,dpi=700)
...
g1 <- ggplot(data=shp_caj)+
  ...
  guides(fill=guide_colorsteps()) +
  scale_fill_gradientn(colours=rev(
    paletteer_c("grDevices::Earth", 30)),breaks=seq(0.1,9500,500)) +theme_bw()+coord_sf(xlim=c(-71.5,-70), ylim=c(-18,-16))+theme(plot.title = element_text(hjust = 0.5,face = "bold",size=14))+
  theme_light()+
  ...
  ggtitle("MSE Kriging y Spline - Chirps : Precipitación")+xlab("")+ylab("")+scale_x_continuous(breaks = seq(-71.5, -70, by = 1), labels = scales::number_format(accuracy = 0.1))+
  annotation_scale(location = "bl", width_hint = 0.28) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),height = unit(0.8, "cm"),
                         width = unit(0.8, "cm"),
...
ggsave(g1,filename=paste0("map_MSE_spline__kriging_pp.png"), width = 10.5, height =10,dpi=700)
